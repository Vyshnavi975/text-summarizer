...
If the key is not set, or the relevant client library isn't
...
def summarize_with_llm(text: str, length: str = "medium") -> str:
    """Summarize using the LLM provider, if a key is configured.

    Raises ``LLMUnavailableError`` if no provider is usable, so callers
    can fall back to the extractive summarizer.
    """
